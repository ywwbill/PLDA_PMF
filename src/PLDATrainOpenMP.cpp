...
int main()
{
	...
	int numTopics=10,numDocs,numWords,numVocab,numIters=500,numTopWords=20;
	...
	//#pragma omp parallel for shared(corpus, docTopicCounts, topicVocabCounts, topicTokenCounts, topicAssigns)
	...
	{
		//sample each doc
		//#pragma omp parallel for schedule(dynamic) private(word,oldTopic,newTopic,sum,sample) shared(corpus,topicScores,topicAssigns,docTopicCounts,topicVocabCounts,topicTokenCounts)
		for (int doc=0; doc<numDocs; doc++)
		{
		...
		//compute theta
		#pragma omp parallel for shared(theta) schedule(dynamic)
		for (int doc=0; doc<numDocs; doc++)
		{
			for (int topic=0; topic<numTopics; topic++)
			{
		...
		logLikelihood=0.0;
		#pragma omp parallel for private(word,sum) reduction(+:logLikelihood) schedule(dynamic)
		for (int doc=0; doc<numDocs; doc++)
		{
			for (unsigned int token=0; token<corpus[doc].size(); token++)
			{
